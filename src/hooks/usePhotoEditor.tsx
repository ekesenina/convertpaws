...
//       canvas.toBlob((blob) => {
//         if (blob) {
//           const newFile = new File([blob], file.name, { type: "webp" });
//           resolve(newFile);
//         } else {
//           resolve(null);
//         }
 ...
 */
  ...
  // Обработка начала касания/зажатия мыши
  const handlePointerDown = (event: React.MouseEvent | React.TouchEvent) => {
    setIsDragging(true);
    const clientX = 'touches' in event ? (event.touches[0] as unknown as Touch).clientX : event.clientX;
    const clientY = 'touches' in event ? (event.touches[0] as unknown as Touch).clientY : event.clientY;
    setPanStart({ x: clientX - offsetX, y: clientY - offsetY });
  
    if ('touches' in event && event.touches.length === 2) {
      // Начальное расстояние для зума
  ...
  // Обработка перемещения касания/мыши
  const handlePointerMove = (event: React.MouseEvent | React.TouchEvent) => {
    if (isDragging && panStart) {
      ...
      if ('touches' in event && event.touches.length === 2 && startDistance) {
        const distance = getDistance(
          event.touches[0] as unknown as Touch,
          event.touches[1] as unknown as Touch
        );
  ...
  // Окончание касания/зажатия мыши
